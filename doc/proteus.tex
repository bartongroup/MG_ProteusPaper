...
visualisation. Interactive analysis is implemented in the \emph{Shiny}
framework, where individual peptides or protein can be examined in the
context of the volcano plot. \emph{Proteus} performs differential
expression with a well-established tool \emph{limma}, offering a robust
treatment of missing data, frequently encountered in label-free
mass-spectrometry experiments. This results in better sensitivity than
random imputation combined with a \emph{t}-test, implemented in a
popular package \emph{Perseus}, as we demonstrate on real and simulated
data. Embedding \emph{Proteus} in R offers access to a wide selection of
statistical and graphical tools for further analysis. Availability and
implementation: The open-source R package, including example data set
and tutorials, is available to install from GitHub
